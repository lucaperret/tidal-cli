...
export interface ResolveResult {
  ...
  stats: { requested: number; resolved: number; duplicatesRemoved: number; truncated: boolean; timedOut: boolean };
  notes: string[];
}
...
export function qualityFromMediaTags(mediaTags?: string[]): AudioQuality {
  if (!mediaTags || mediaTags.length === 0) return 'UNKNOWN';
  const up = mediaTags.map((t) => String(t).toUpperCase());
  if (up.some((t) => HIRES_TAGS.has(t))) return 'HI_RES_LOSSLESS';
  if (up.some((t) => LOSSLESS_TAGS.has(t))) return 'LOSSLESS';
  return 'UNKNOWN';
}

export function isLossless(q: AudioQuality): boolean {
...
function contains(a: string, b: string, minLen: number): boolean {
...
function titleScore(queryTitle: string, candidateTitle: string): number {
  ...
  if (!q || !c) return 0;
  if (q === c) return 1;
  if (coreTitle(queryTitle) === coreTitle(candidateTitle)) return 0.9;
  if (contains(q, c, 4)) return 0.7;
  return tokenOverlap(q, c) * 0.6;
}

function artistScore(queryArtist: string, candidateArtists: string[]): number {
  ...
  for (const a of candidateArtists) {
    ...
    else if (contains(q, c, 3)) best = Math.max(best, 0.8);
    else best = Math.max(best, tokenOverlap(q, c) * 0.6);
  }
  return best;
}
...
function mapIncludedTracks(data: any): RawTrack[] {
  const included = data?.included ?? [];
  const artistName = new Map<string, string>();
  for (const i of included) {
    if (i?.type === 'artists') artistName.set(String(i.id), i.attributes?.name ?? '');
  }
  return included
    ...
    .map((t: any) => {
      ...
      const artists = rels.map((r: any) => artistName.get(String(r.id))).filter((n: any): n is string => !!n);
      return {
        id: String(t.id),
        title: a.title ?? '',
        artists,
...
interface RawSearchResult {
  tracks: RawTrack[];
  /** True on a rate-limit / server / network error — distinct from a genuine zero-result. */
  transient: boolean;
}
...
async function searchTracksRaw(client: any, countryCode: string, query: string): Promise<RawSearchResult> {
  try {
    const res = await client.GET('/searchResults/{id}', {
      params: { path: { id: query }, query: { countryCode, include: ['tracks', 'artists'] } },
    });
    if (res.error || !res.data) return { tracks: [], transient: isTransientStatus(res.error?.status) };
...
async function lookupByIsrcRaw(client: any, countryCode: string, isrc: string): Promise<RawSearchResult> {
  let data: any;
  try {
...
async function verifyQualityViaManifest(client: any, trackId: string): Promise<AudioQuality> {
  try {
    ...
    const formats: string[] = res.data?.data?.attributes?.formats ?? [];
...
function pickBest(
...
): { track: RawTrack; match: ResolvedTrack['match'] } | null {
  const EPS = 1e-9;
  let best: RawTrack | null = null;
  ...
  const match: ResolvedTrack['match'] =
    bestScore >= 0.9 && (bestHasArtist || !query.artist) ? 'exact' : bestScore >= 0.7 ? 'strong' : 'weak';
  return { track: best, match };
}

type ResolveOutcome = { track: ResolvedTrack } | { track: null; transient: boolean };
...
async function resolveOne(
  ...
  if (!query.title || !query.title.trim()) return { track: null, transient: false };
  ...
  if (!best) return { track: null, transient };
...
export async function resolveTracks(
  ...
  const outcomes = new Array<ResolveOutcome | undefined>(slice.length); // undefined = not attempted
  let timedOut = false;
  let nextIdx = 0;
  ...
  const notFound: UnresolvedQuery[] = [];
  ...
  const filteredOut: FilteredTrack[] = [];
  for (const t of deduped) {
  ...
  if (opts.losslessOnly) {
    ...
    if (kept.length === 0 && unconfirmed > 0 && unconfirmed === filteredOut.length) {
      ...
      notes.push(
        'No tracks could be confirmed as lossless — the Tidal search response carried no audio-quality tags for these results. Re-run with verification enabled to check each track via playback manifests.',
      );
    } else if (unconfirmed > 0) {
      notes.push(
        `${unconfirmed} track${unconfirmed === 1 ? '' : 's'} excluded because lossless status could not be confirmed from metadata — re-run with verification enabled to check via playback manifests.`,
      );
    }
  }
 ...
 */
export function buildPreviewSummary(r: ResolveResult, opts: { mode?: string; name?: string } = {}): string {
  const lines: string[] = [];
  const label = opts.name ? `"${opts.name}"` : 'Playlist';
  const modeNote =
    opts.mode === 'audiophile' ? ' (lossless-only)'
  ...
  if (r.tracks.length > 10) lines.push(`  …and ${r.tracks.length - 10} more.`);
